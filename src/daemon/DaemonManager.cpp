...
{
    // prepare command line arguments and pass to leviard
    ...
    qDebug() << "starting leviard " + m_monerod;
    ...
    while(true && !m_app_exit) {
        ...
        if(running(nettype)) {
            ...
            if(counter >= 5) {
                qDebug() << "Killing it! ";
#ifdef Q_OS_WIN
                QProcess::execute("taskkill /F /IM leviard.exe");
#else
                QProcess::execute("pkill leviard");
#endif
            }
...
{ 
    ...
    qDebug() << status;
    // `./leviard status` returns BUSY when syncing.
    ...
    : QObject(parent)
{
    ...
    m_monerod = QApplication::applicationDirPath() + "/leviard";
#endif
